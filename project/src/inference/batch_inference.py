...
def process_folder(input_dir):
    ...
    for img_name in tqdm(images, desc="Pro Batch Analysis"):
        ...
        # Overlays
        cv2.putText(combined, f"{city} | {lat:.2f}, {lon:.2f} | Rain: {rain}mm", (20, 30), 1, 1.5, (0, 255, 0), 2)
        cv2.putText(combined, f"Water: {water_p}% | {severity}", (20, 65), 1, 1.5, (255, 255, 0), 2)
        if severity == "HIGH":
        ...
        report_data.append({"file": img_name, "water": water_p, "severity": severity, "city": city, "lat": lat, "lon": lon, "rain": rain})

    # Save Reports
    report_dir = os.path.join(OUTPUT_DIR, "reports")
    os.makedirs(report_dir, exist_ok=True)
    with open(os.path.join(report_dir, "pro_batch_report.csv"), "w", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=["file", "water", "severity", "city", "lat", "lon", "rain"])
        writer.writeheader()
